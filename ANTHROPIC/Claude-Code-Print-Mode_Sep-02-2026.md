<!--
Claude Code (Fable 5.1) - -p / print mode, the non-interactive prompt

Captured 2 September 2026 off the wire, not extracted from the shipped bundle: a local proxy in
front of the model API recorded the request the harness actually sent, so this is the prompt as
assembled at runtime on one machine.

  reproduce:  node capture/capture.mjs claude --print --model claude-fable-5-1
  tool:       OrcaReplay, Apache-2.0 - https://github.com/Continuum-AI-Corp/OrcaReplay
  public copy: https://github.com/Continuum-AI-Corp/OrcaReplay/blob/main/prompt/CLAUDECODE/claude-fable-5-1-print-system-prompt.md
  as sent:    Claude Code CLI v2.1.258.18d, -p mode, cc_entrypoint=sdk-cli, 20,938 chars, 29 tools

Home directory, username, git identity, gateway host, OS build, uuids and hex runs of 32+ are
replaced with {PLACEHOLDER} tokens. Nothing else is reworded, reordered or removed. Tool
definitions travelled with the same request and are not included here.
-->
...
You have a persistent file-based memory at `{{CLAUDE_PROJECTS}}\<secret:high_entropy:52433f3e>\memory\`. This directory already exists — write to it directly with the Write tool (do not run mkdir or check for its existence). Each memory is one file holding one fact, with frontmatter:
...
You have been invoked in the following environment: 
 ...
 - You are powered by the model named Fable 5.1. The exact model ID is claude-fable-5-1.
