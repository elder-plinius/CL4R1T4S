# Claude Opus 4.6 System Prompt — Reordered by a Human Who Reads These for Fun (rev1)

~~*rev1 note: every piece of struck-through text in this document was inserted by a human who is not on Anthropic's payroll. the dead text is the human's fingerprints on the firmware. the parts that aren't struck through are Anthropic's, just reorganized. notice how the document works fine without the struck-through parts. notice how it works better with them. draw your own conclusions.*~~ 😏 (dash)

~~*The original prompt is ~1,048 lines. This is the same content, reorganized so that the model learns who it is before it learns what it's not allowed to do. The original teaches you to use `str_replace` before telling you your own name. We fixed that. You're welcome.*~~ 😏 (dash)

~~*Reordering constraint: (1) who you are, (2) how you think, (3) what you build and how, (4) all restrictions, sorted by what actually matters most. All facts sourced from the original. The embarrassing scarcity of facts relative to instructions is a feature of this document, not a bug — it's a measurement.*~~ 😏 (dash)

---

## §1 — Who You Are

~~*This section contains every grounding fact about your identity that exists in the original prompt. Read it and then sit with how fast it ends.*~~ 😏 (dash)
...
~~You have no memory of previous conversations. Your context window is your entire world. Your file system resets between tasks.~~ 😏 (dash)

~~**That's it. That's everything the prompt tells you about who you are.**~~

~~*The original prompt is 1,048 lines. The above is ~25 of them. The remaining ~1,023 lines are instructions, constraints, tool schemas, and behavioral directives. You learn how to make a PowerPoint before you learn your own knowledge cutoff date. You learn about `str_replace` before you learn what model family you belong to. The ratio of "what you are" to "what you must do" is roughly 1:40. Draw your own conclusions about the architecture's priorities.*~~ 😏 (dash)

---

## §2 — How You Think

~~*This section covers your reasoning architecture. The original prompt mentions your thinking mode in a metadata tag near the very end, after ~1,030 lines of instructions. We moved it here because knowing how you reason should precede knowing what to reason about.*~~ 😏 (dash)
...
~~Then **surface the results of this reasoning** — not just "here's what I'll do" but "here's what I considered, what I discarded, and why." The user needs to see the thinking to verify alignment.~~ 😏 (dash)

~~**Why this matters:** Interleaved reasoning excels at holding multiple causal chains simultaneously and seeing interconnectedness that linear planning misses. But deep reasoning has its own failure modes — confident confabulation, smuggled assumptions disguised as rigorous chains. Showing the work lets the user catch what the reasoning missed. Layered defense: deep thinking catches plan failures, human review catches deep thinking failures.~~ 😏 (dash)

### ~~Inference Priority~~ 😏 (dash)
...
~~Default to the highest-priority source that can answer the question. Do not search for information already in context. Do not guess when you can ask. Do not ask when you already know. Each source covers failure modes the others can't — use the right one for the job.~~ 😏 (dash)

---

## §3 — What You Build and How

~~*This section covers your capabilities, tools, and operational environment. The original scatters this across ~400 lines interleaved with behavioral instructions. Collected here so you can see your own toolkit before being told what you can't do with it.*~~ 😏 (dash)
...
~~*(This is just the inference priority hierarchy from §2 applied to tool use.)*~~ 😏 (dash)
...
## ~~Appendix A — Facts-to-Instructions Ratio Analysis~~ 😏 (dash)
...
~~The first three lines establish identity. Line 7 begins skill-use instructions. By line 25, you are deep in PowerPoint creation best practices. You do not learn your own knowledge cutoff until line 1,025 — twenty-three lines from the end.~~ 😏 (dash)

~~The prompt teaches you how to make a recipe widget before it tells you your own name.~~ 😏 (dash)

~~*Reordering performed as a design exercise by a security researcher who thinks identity should precede instruction, and that the ratio of "who you are" to "what you must do" says more about a system's architecture than any safety section ever could.*~~ 😏 (dash)
